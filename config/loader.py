...
class PersonalMode(ModeConfig):
    ...
    def _get_or_create_secret_key(self) -> str:
        ...
        if not key:
            ...
            logger.info("Generated session secret key (set SECRET_KEY env var for persistence)")
        return key
...
class CloudMode(ModeConfig):
    ...
    def __post_init__(self):
        ...
        self.security = SecurityConfig(
            rate_limiting_enabled=True,
            rate_limit_per_minute=int(os.environ.get('RATELIMIT_PER_MINUTE', '60')),
            rate_limit_storage_url=os.environ.get('RATELIMIT_STORAGE_URL', 'redis://localhost:6379'),
...
def detect_mode() -> ModeConfig:
    ...
    if os.environ.get('OPENGLAZE_CLOUD', '').lower() in ('true', '1', 'yes'):
        logger.info("Cloud mode requested via OPENGLAZE_CLOUD environment variable (legacy)")
        try:
            return CloudMode()
        except ConfigurationError as e:
            logger.error(f"Cloud mode configuration error: {e}")
...
def _substitute_env_vars(content: str) -> str:
    ...
    def replacer(match):
        ...
        if any(sensitive in var_name.upper() for sensitive in
