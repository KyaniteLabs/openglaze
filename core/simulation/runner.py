...
def simulate_combo(base_glaze, top_glaze, combo, db_path: str = 'glaze.db') -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return _fallback_prediction_with_umf(base_glaze, top_glaze, combo, base_umf, top_umf, compat)
    ...
    try:
        ...
        if base_recipe:
            ...
            if not base_umf.success:
                logger.debug(f"Could not calculate UMF for base {combo.base}: {base_umf.error}")

        if top_recipe:
            top_umf = calculate_umf(top_recipe)
            if not top_umf.success:
                logger.debug(f"Could not calculate UMF for top {combo.top}: {top_umf.error}")
...
def _compatibility_context(compat) -> str:
    ...
    parts.append(f"Compatibility score: {compat.score:.0%} ({'compatible' if compat.compatible else 'incompatible'})")
...
def _fallback_prediction(base_glaze, top_glaze, combo, base_umf=None, top_umf=None, compat=None) -> Dict[str, Any]:
    """Generate a rule-based prediction when AI is not available."""
    return _fallback_prediction_with_umf(base_glaze, top_glaze, combo, base_umf, top_umf, compat)


def _fallback_prediction_with_umf(base_glaze, top_glaze, combo, base_umf=None, top_umf=None, compat=None) -> Dict[str, Any]:
    ...
    if compat and compat.success:
        ...
        explanation = 'UMF-based fallback analysis: ' + '; '.join(parts) if parts else 'UMF-based fallback'
        predicted = f"Compatibility score {compat.score:.0%} — " + (
            "glazes appear compatible for layering" if compat.compatible
        ...
        return {
            ...
            'confidence_in_prediction': 'high' if grade in ('likely', 'unlikely') else 'medium',
        }
    ...
    elif ('tenmoku' in base_name and 'tenmoku' in top_name) or \
         ('teadust' in base_name and 'tenmoku' in top_name) or \
         ('tenmoku' in base_name and 'teadust' in top_name):
        grade = 'unlikely'
    ...
    elif 'white' in base_name and ('blue' in top_name or 'jensen' in top_name or 'aegean' in top_name):
        grade = 'likely'
        predicted = "Cobalt-based blue over white — most predictable combination in ceramics."
    elif 'white' in base_name and ('froggy' in top_name or 'toady' in top_name or 'blugr' in top_name):
        grade = 'likely'
