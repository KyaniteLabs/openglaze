...
class BatchAnalyzer:
    ...
    def generate_report(self) -> dict:
        ...
        combo_compatible = sum(1 for r in combo_results.values() if r.get('compatible') is True)
        combo_incompatible = sum(1 for r in combo_results.values() if r.get('compatible') is False)
        ...
        scores = [r.get('score', 0) for r in combo_results.values() if r.get('score') is not None]
