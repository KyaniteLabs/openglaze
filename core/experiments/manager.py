...
class ExperimentManager:
    ...
    ALLOWED_COLUMNS = {
        'combination_id', 'base_glaze', 'top_glaze', 'stage', 'status',
        'prediction', 'result', 'rating', 'notes', 'photo', 'completed_at',
        'clay_body', 'firing_type', 'cone', 'confirmation', 'fired_at',
        'application_method', 'thickness', 'drying_time', 'cooling_notes',
        'atmosphere_notes', 'fired_by', 'studio_id'
    }
    ...
    def get_all(self) -> List[Experiment]:
        ...
        if self.user_id:
            ...
            cursor.execute('SELECT * FROM experiments WHERE user_id IS NULL ORDER BY created_at DESC')
    ...
    def get_active(self) -> List[Experiment]:
        ...
        if self.user_id:
            ...
            cursor.execute("SELECT * FROM experiments WHERE status = 'in_progress' AND user_id IS NULL")
    ...
    def get_by_stage(self, stage: str) -> List[Experiment]:
        ...
        if self.user_id:
            ...
            cursor.execute('SELECT * FROM experiments WHERE stage = ? AND user_id IS NULL', (stage,))
    ...
    def get_by_id(self, exp_id: int) -> Optional[Experiment]:
        ...
        if self.user_id:
            ...
            cursor.execute('SELECT * FROM experiments WHERE id = ? AND user_id IS NULL', (exp_id,))
    ...
    def create(self, experiment: Experiment) -> int:
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            experiment.combination_id, experiment.base_glaze, experiment.top_glaze,
            experiment.stage, experiment.status, experiment.prediction,
            experiment.result, experiment.rating, experiment.notes,
            experiment.photo, self.user_id
        ))
    ...
    def advance_stage(self, exp_id: int) -> bool:
        ...
        stages = ['ideation', 'prediction', 'application', 'firing', 'analysis', 'documentation']
        current_idx = stages.index(experiment.stage) if experiment.stage in stages else 0

        if current_idx >= len(stages) - 1:
            # Already at documentation, mark as completed
            return self.update(exp_id, {
                'status': 'completed',
    ...
    def record_result(self, exp_id: int, result: str, rating: int = None, photo: str = None) -> bool:
    ...
    def delete(self, exp_id: int) -> bool:
        ...
        if self.user_id:
            cursor.execute('DELETE FROM experiments WHERE id = ? AND user_id = ?', (exp_id, self.user_id))
        else:
            cursor.execute('DELETE FROM experiments WHERE id = ? AND user_id IS NULL', (exp_id,))
    ...
    def get_pipeline_stats(self) -> dict:
        ...
        # Total count
        if self.user_id:
            cursor.execute('SELECT COUNT(*) as count FROM experiments WHERE user_id = ? OR user_id IS NULL', (self.user_id,))
        else:
            cursor.execute('SELECT COUNT(*) as count FROM experiments WHERE user_id IS NULL')
        ...
        for stage in stages:
            if self.user_id:
                ...
                cursor.execute('SELECT COUNT(*) as count FROM experiments WHERE stage = ? AND user_id IS NULL', (stage,))
        ...
        for status in statuses:
            if self.user_id:
                ...
                cursor.execute("SELECT COUNT(*) as count FROM experiments WHERE status = ? AND user_id IS NULL", (status,))
    ...
    def log_firing_result(self, exp_id: int, log_data: dict) -> bool:
        ...
        # Optional fields
        for key in ('application_method', 'thickness', 'drying_time', 'cooling_notes',
                     'atmosphere_notes', 'fired_by', 'result', 'rating', 'notes', 'photo'):
