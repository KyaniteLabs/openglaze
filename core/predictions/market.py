...
def generate_ai_prediction(combo_info) -> tuple:
    ...
    # High risk pairs
    high_risk_pairs = [
        ('shino', 'shino'), ('crawl', 'crawl'),
        ('tenmoku', 'tenmoku'), ('ash', 'ash')
    ]
    for pair in high_risk_pairs:
        if pair[0] in glaze_a and pair[1] in glaze_b:
            return ("High risk of crawling or running due to overlapping flux chemistry. "
                    "Consider thin application."), 25

    # Matte + glossy contrast
    if ('matte' in surface_a and 'smooth' in surface_b) or \
       ('smooth' in surface_a and 'matte' in surface_b):
        return ("Matte over glossy (or vice versa) typically creates interesting "
                "surface contrast. Good compatibility expected."), 70

    # Clear base combinations
    if 'clear' in glaze_a or 'clear' in glaze_b:
        return ("Clear base glaze combinations are generally reliable. The underlying "
                "glaze color will show through clearly."), 75
    ...
    if color_a and color_b and color_a == color_b:
        return ("Similar color pairing may lack contrast. Consider pairing with a "
                "complementary color glaze."), 40

    # General prediction
    return ("Compatible combination based on standard oxide chemistry. Results may vary "
            "with application thickness and firing conditions."), 55
