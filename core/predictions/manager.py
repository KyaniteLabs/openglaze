...
class PredictionManager:
    ...
    def create_prediction(self, user_id: str, combo_id: int,
                          predicted_outcome: str, confidence: int = 50) -> dict:
    ...
    def resolve_predictions(self, combo_id: int, status: str) -> list:
        ...
        for pred in cursor.fetchall():
            ...
            resolved.append({'prediction_id': pred['id'], 'correct': correct, 'points': points})
    ...
    def get_leaderboard(self, user_id: str) -> dict:
        ...
        if user_stats and user_stats['predictions_total'] > 0:
            user_acc = user_stats['predictions_correct'] / user_stats['predictions_total'] * 100
