...
try:
    ...
    logger.warning("Auth module not available - cloud mode disabled")
...
def create_app(config: dict = None) -> Flask:
    ...
    def promote_combination(combo_id: int):
        ...
        if success:
            ...
            if user_id and GAMIFICATION_AVAILABLE:
                g.gamification_manager.on_combination_tested(user_id, combo_id, 'proven')
                g.gamification_manager.log_activity(user_id, 'combination_tested', {'combination_id': combo_id})
    ...
    def simulate_combination(combo_id: int):
        ...
        try:
            ...
            g.combo_manager.update(combo_id, {
    ...
    def upload_photo():
        ...
        try:
            relative_path = save_uploaded_file(file, os.path.join('frontend', 'uploads'))
            return jsonify({
                "success": True,
                "path": relative_path,
                "url": f"/{relative_path}"
    ...
    def get_inbox():
        ...
        if user_id:
            cursor.execute(
                'SELECT * FROM inbox WHERE user_id = ? OR user_id IS NULL',
                (user_id,)
            )
        else:
    ...
    def add_to_inbox():
        ...
        cursor.execute(
            ...
            (data.get('base'), data.get('top'), data.get('notes'), get_current_user_id())
        )
        conn.commit()
    ...
    @app.route('/api/studios/<studio_id>/lab-queue/<int:combo_id>/claim', methods=['POST'])
    def claim_combo(studio_id, combo_id):
        ...
        assignment = g.studio_manager.claim_combo(studio_id, combo_id, user_id, display_name)
        if not assignment:
            return jsonify({"error": "Already claimed by someone else"}), 409

        return jsonify(assignment.to_dict()), 201

    @app.route('/api/studios/<studio_id>/lab-queue/<int:combo_id>/release', methods=['POST'])
    ...
    def submit_firing_log(exp_id):
        ...
        if exp and exp.combination_id and confirmation in ('confirmed', 'surprise'):
            g.combo_manager.update(exp.combination_id, {
            ...
            if data.get('studio_id') and STUDIOS_AVAILABLE:
                g.studio_manager.complete_claim(data['studio_id'], exp.combination_id, exp_id)
    ...
    def create_prediction():
        ...
        try:
            result = g.prediction_manager.create_prediction(user_id, combo_id, predicted_outcome, confidence)
            resp = jsonify(result)
    ...
    def get_progress():
        ...
        try:
            ...
            # Combo counts
            cursor.execute('SELECT COUNT(*) FROM combinations WHERE user_id = ?', (user_id,))
            total_combos = cursor.fetchone()[0]

            cursor.execute("SELECT COUNT(*) FROM combinations WHERE user_id = ? AND stage IN ('tested', 'proven', 'confirmed')", (user_id,))
            tested_combos = cursor.fetchone()[0]

            cursor.execute("SELECT COUNT(*) FROM combinations WHERE user_id = ? AND type = 'research-backed'", (user_id,))
            research_combos = cursor.fetchone()[0]

            cursor.execute("SELECT COUNT(*) FROM combinations WHERE user_id = ? AND type = 'user-prediction'", (user_id,))
            prediction_combos = cursor.fetchone()[0]

            # Stage breakdown
            cursor.execute('SELECT stage, COUNT(*) FROM combinations WHERE user_id = ? GROUP BY stage', (user_id,))
        ...
        try:
            ...
            for glaze in template.get('glazes', []):
                ...
                cursor.execute('''INSERT OR IGNORE INTO glazes
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                    (glaze_id, glaze.get('name'), glaze.get('family'), glaze.get('hex'),
                     glaze.get('chemistry'), glaze.get('behavior'), glaze.get('layering'),
                     glaze.get('warning'), glaze.get('recipe'), glaze.get('catalog_code'),
                     glaze.get('food_safe'), glaze.get('notes'), user_id))
...
def _run_migrations(conn):
    ...
    cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='combinations'")
    if not cursor.fetchone():
        return

    # Migration 1: Rename type values
    try:
        cursor.execute("UPDATE combinations SET type = 'research-backed' WHERE type = 'proven'")
        cursor.execute("UPDATE combinations SET type = 'user-prediction' WHERE type = 'hypothesis'")
    ...
    if 'stage' not in columns:
        ...
        cursor.execute("UPDATE combinations SET stage = 'documented' WHERE type = 'research-backed' AND stage = 'idea'")

    # Migration 3: Add prediction_grade column if missing
    if 'prediction_grade' not in columns:
        cursor.execute("ALTER TABLE combinations ADD COLUMN prediction_grade TEXT DEFAULT 'unknown'")
...
def seed_database(db_path: str):
    ...
    for c in template.get('research_backed_combinations', []):
        ...
        cursor.execute('''INSERT INTO combinations (base, top, layers, type, source, result, chemistry, risk, stage, prediction_grade)
            VALUES (?, ?, ?, 'research-backed', ?, ?, ?, ?, 'documented', ?)''',
            (c.get('base'), c.get('top'), layers_json,
             c.get('source'), c.get('result'), c.get('chemistry'), c.get('risk'),
    ...
    chemistry_rules = [
        # Color prediction (8)
        ("color_prediction", "oxide_atmosphere", "Iron 2% + oxidation = amber/tan",
         "Iron oxide at ~2% in an oxidation (oxygen-rich) atmosphere produces warm amber to tan colors.",
         "Iron oxide 2%, oxidation atmosphere", "Amber, tan, or light brown",
         ...
         "high"),
        ("color_prediction", "oxide_atmosphere", "Iron 2% + reduction = celadon green",
         "Iron oxide at ~2% in a reduction (oxygen-depleted) atmosphere produces the classic celadon green.",
         "Iron oxide 2%, reduction atmosphere", "Celadon green to blue-green",
         ...
         "high"),
        ("color_prediction", "oxide_atmosphere", "Copper 1% + oxidation = green",
         "Copper oxide at ~1% in oxidation produces reliable greens ranging from mint to forest green.",
         "Copper oxide 1%, oxidation atmosphere", "Green (mint to forest)",
         ...
         "high"),
        ("color_prediction", "oxide_atmosphere", "Copper 1% + heavy reduction = copper red",
         ...
         "medium"),
        ("color_prediction", "oxide_stable", "Cobalt 0.5-2% = blue in any atmosphere",
         ...
         "Cobalt oxide 0.5-2%, any atmosphere", "Blue — from pale sky blue (0.5%) to deep cobalt blue (2%)",
        ...
        ("color_prediction", "oxide_atmosphere", "Manganese + reduction = metallic brown/black",
         ...
         "Manganese dioxide, reduction atmosphere", "Metallic brown to black",
         ...
         "high"),
        ("color_prediction", "oxide_interaction", "Chrome + tin = pink",
         "Chromium oxide combined with tin oxide produces pink. This is a classic ceramic color interaction.",
         "Chromium oxide + tin oxide in base glaze", "Pink to mauve",
        ...
        ("layering_principle", "compatibility", "High iron over high iron = too dark",
         ...
         "Two glazes both with >2% iron oxide", "Very dark brown to black, loss of individual character",
         "Some potters intentionally create dark moody surfaces this way. If that's the goal, lean into it.",
         "high"),
        ("layering_principle", "atmosphere_sensitivity", "Copper glazes are sensitive to reduction",
         ...
         "Any glaze containing copper, variable atmosphere", "Green in oxidation, red/metallic/clear in reduction",
         ...
         "high"),
        ("layering_principle", "layering_effect", "Clear glazes add depth without changing color",
         ...
         "Clear transparent glaze over colored glaze", "Deeper, glossier version of base color",
         "The clear must be truly transparent. Some 'clears' have a slight blue or yellow cast that will shift the base color.",
         "high"),
        ("layering_principle", "layering_effect", "White opaque glazes can cover dark bases",
         ...
         "White opaque glaze over dark glaze", "Lightened base color to full white coverage depending on thickness",
         ...
         "high"),
        ("layering_principle", "compatibility", "Shino over non-Shino = high risk crawling",
         ...
         "Shino glaze over non-Shino base", "MODERATE to HIGH risk of crawling (glaze pulls away from surface in patches)",
        ...
        ("common_issue", "defect", "Crazing",
         ...
         "high"),
        ("common_issue", "defect", "Thermal shock / dunting",
        ...
        ("atmosphere_effect", "oxidation", "Oxidation atmosphere effects",
         ...
         "Oxidation atmosphere (air damper open, plenty of oxygen)", "Iron: amber/tan/yellow. Copper: green. Cobalt: blue. Manganese: purple/brown.",
         "Oxidation is generally more predictable and forgiving than reduction. Good for functional ware where consistency matters.",
         "high"),
        ("atmosphere_effect", "reduction", "Reduction atmosphere effects",
         ...
         "Reduction atmosphere (damper partially closed, fuel-rich)", "Iron: celadon green to gray. Copper: can go red/metallic/clear. Surfaces: more varied, often more complex.",
        ...
        ("application_effect", "thickness", "Thin application effects",
         ...
         "Glaze applied in 1-2 thin coats", "Lighter color, more texture showing through, lower defect risk",
         ...
         "high"),
        ("application_effect", "thickness", "Thick application effects",
         ...
         "Glaze applied in 3+ coats or very thick single coat", "Deeper/richer color, more pooling and variation, higher running and crawling risk",
        ...
        # Firing effects (2)
        ("firing_effect", "speed", "Fast firing effects",
         "Fast firing (quick temperature rise) produces more varied and textured surfaces but increases defect risk.",
         "Firing rate: fast (several hours to cone 10)", "More texture and variation, higher defect risk (pinholing, bloating, cracking)",
         ...
         "medium"),
        ("firing_effect", "cooling", "Slow cooling effects",
         ...
         "Controlled slow cooling, optional hold at 1000-1500°F", "Smoother surface, better crystal growth, more consistent color, reduced cracking risk",
        ...
        ("layering_principle", "material_interaction", "Zinc affects multiple colorants",
         ...
         "Zinc oxide present in base glaze", "Chrome-tin pinks turn brown; copper color development can shift; iron greens may become lighter or browner",
         ...
         "high"),
        ("application_effect", "material_property", "Boron as a powerful flux",
         ...
         "Boron-containing materials in glaze", "Lower melting temperature, potential color shifts, increased running risk",
        ...
        # Material safety (12)
        ("material_safety", "dust_hazard", "Silica dust — respirable crystalline silica",
         ...
         "Mixing dry glaze materials, sanding fired pieces, cleaning up dry spills", "Silicosis risk with chronic exposure; irritation of eyes, nose, throat with acute exposure",
         ...
         "high"),
        ("material_safety", "fume_hazard", "Manganese fumes — toxic above 1900°F",
         ...
         "Firing glazes containing manganese dioxide, especially above 1900°F", "Manganism with chronic exposure: tremors, muscle rigidity, mood changes; acute irritation of respiratory tract",
         ...
         "high"),
        ("material_safety", "toxicity", "Barium toxicity",
         ...
         "Glazes containing barium carbonate, especially functional ware", "Soluble barium is toxic if ingested — affects heart, muscles, nerves; can cause cardiac arrest in severe cases",
         ...
         "high"),
        ("material_safety", "extreme_toxicity", "Lead — extremely toxic, never for functional ware",
         ...
         "Any glaze containing lead compounds (lead bisilicate, red lead, lead frits)", "Neurological damage, developmental delays in children, organ damage, death in severe cases",
         ...
         "high"),
        ("material_safety", "carcinogen", "Cadmium — carcinogen, avoid for functional ware",
         ...
         "Glazes containing cadmium compounds (cadmium red, cadmium yellow)", "Lung cancer risk with inhalation; kidney damage; bone disease (Itai-Itai disease with chronic exposure)",
         ...
         "high"),
        ("material_safety", "valence_toxicity", "Chromium VI vs III — oxidation state matters",
         ...
         "Chromium oxide (Cr2O3) in glazes — this is Cr(III)", "Cr(VI): carcinogenic, causes lung cancer, skin ulcers; Cr(III) in glazes: generally safe, low toxicity",
         ...
         "high"),
        ("material_safety", "sensitizer", "Nickel — skin sensitizer and inhalation hazard",
         ...
         "Mixing dry glazes containing nickel oxide or nickel compounds", "Allergic contact dermatitis (skin rash); respiratory irritation with inhalation; possible carcinogen with chronic exposure",
         ...
         "medium"),
        ("material_safety", "toxicity", "Lithium — toxic if ingested, handle with gloves",
         ...
         "Handling lithium carbonate or lithium-bearing feldspars", "Nausea, tremors, confusion with ingestion; skin irritation with prolonged contact",
         ...
         "medium"),
        ("material_safety", "ppe", "Respirator requirements for pottery",
         ...
         "Any dry material handling: mixing glazes, sanding, cleaning dust", "N95 minimum for mixing dry glazes; P100 for sanding fired pieces; full facepiece with P100 cartridges for spraying glazes",
         ...
         "high"),
        ("material_safety", "ventilation", "Kiln ventilation requirements",
         ...
         "Any kiln firing, especially reduction firing and glazes with toxic materials", "Minimum 100-150 fpm face velocity for hood capture; never fire in enclosed living space; cross-ventilation for gas kilns",
         ...
         "high"),
        ("material_safety", "cleanup", "Cleanup safety — wet methods only",
         ...
         "Cleaning studio surfaces, glaze spills, clay scraps", "Wet mop or damp sponge for floors; wet wipe for work surfaces; HEPA vacuum if dry cleaning is unavoidable",
         ...
         "high"),
        ("material_safety", "high_exposure", "Glaze dust during sanding — full PPE required",
         ...
         "Sanding fired glaze surfaces to smooth rough spots, remove kiln shelf residue", "High exposure to respirable crystalline silica and metal oxide dusts",
        ...
        # Food safety (8)
        ("food_safety", "leaching", "Food safety definition — leaching and bacterial risk",
         ...
         "Any glazed vessel intended for food or beverage contact", "Safe glaze: fully mature, smooth surface, no toxic colorants leaching, passes acid test",
         ...
         "high"),
        ("food_safety", "defect_risk", "Crazing = not food safe for functional ware",
         ...
         "Glaze with visible crackle pattern, used for food/beverage", "Bacteria growth in crack network; staining over time; potential food contamination",
         ...
         "high"),
        ("food_safety", "toxic_colorants", "Toxic colorants to avoid for functional ware",
         ...
         "Functional ware glazes containing: lead (any amount), cadmium (any amount), high barium (>5%), high manganese (>5%)", "Potential metal leaching into food/beverage, especially under acidic conditions",
         ...
         "high"),
        ("food_safety", "acid_leaching", "Acid resistance — citrus and vinegar leach metals",
         ...
         "Glazed vessel used for acidic food or drink (lemon juice, vinegar, coffee, wine, tomato)", "Increased leaching of barium, copper, manganese, cadmium, and other metals under acidic conditions",
         ...
         "high"),
        ("food_safety", "testing", "Food safety testing methods",
         ...
         "Any glaze intended for food/beverage use", "Home lead test kits: quick screening for lead only, limited accuracy; Lab ICP-MS testing: comprehensive metal analysis, 100-200 USD per sample",
         ...
         "high"),
        ("food_safety", "commercial_assumptions", "Commercial glaze food safety claims",
         ...
         "Using commercially prepared glazes for functional ware", "Manufacturer claims are a good starting point but not a guarantee of safety",
         ...
         "medium"),
        ("food_safety", "maturity", "Glaze maturity and leaching",
         ...
         "Glaze fired below its intended temperature (underfired by even half a cone)", "Dramatically increased metal leaching from underfired glazes; surface may appear properly melted but not be fully reacted",
         ...
         "high"),
        ("food_safety", "surface_texture", "Surface texture and bacterial harboring",
         ...
         "Glaze with rough texture, crawling, pinholing, or other surface defects on food-contact surface", "Bacteria can lodge in surface irregularities and resist normal washing",
        ...
        ("firing_effect", "cooling", "Copper red critical cooling window",
         ...
         "Copper red glaze at cone 10 reduction", "Hold at 1500-1800°F for 30-60 minutes, then controlled cooling; skipping this step almost always produces green",
         ...
         "high"),
        ("firing_effect", "cooling", "Crystal glaze slow cooling requirements",
         ...
         "Crystal glazes containing zinc and silica in specific proportions", "Very slow cooling through 1800-1400°F (100-200°F per hour); crystal size and number increase with slower cooling and longer holds",
         ...
         "high"),
        ("firing_effect", "cooling", "Shino carbon trapping timing",
         ...
         "Shino glazes in reduction firing", "Soda melts around 1560°F (850°C); carbon trapping occurs as the glaze begins to melt and trap carbon from the reduction atmosphere",
         ...
         "medium"),
        ("firing_effect", "cooling", "Iron crystallization during cooling",
         ...
         "High-iron glazes (5%+) with slow cooling", "Slow cooling promotes iron crystal growth visible as green/gold/brown spots; faster cooling keeps iron dissolved for solid colors",
         ...
         "high"),
        ("firing_effect", "cooling", "Tea dust specific cooling cycle",
         ...
         "Tea dust or high-iron glazes with zinc", "Controlled slow cooling through 1800-1400°F; hold or very slow ramp allows iron crystals to grow; typical cycle: natural cool to 1800°F, then 100°F/hour to 1400°F",
         ...
         "high"),
        ("firing_effect", "cooling", "Quartz inversion zone — critical for preventing dunting",
         ...
         "All ceramic pieces, especially thick or uneven sections", "Cool slowly through 1063°F (573°C); pieces crack or shatter if cooled too fast through this zone",
         ...
         "high"),
        ("firing_effect", "cooling", "Cristobalite conversion — delayed cracking risk",
         ...
         "Pieces that have developed cristobalite during firing (common at cone 10 with long soaks)", "Cool slowly through 437°F (225°C); rapid cooling can cause spontaneous cracking days or weeks after firing",
         ...
         "medium"),
        ("firing_effect", "cooling", "Crash cooling effects on glaze surface",
         ...
         "Any glaze with crash cooling or rapid cool", "Glossier, more glass-like surface; no crystal development; faster turnaround; increased risk of thermal shock cracking",
        ...
        ("application_effect", "method", "Dip application — most uniform thickness",
         ...
         "Glaze mixed to dipping consistency (specific gravity ~1.4-1.5), piece submerged for 3-5 seconds", "Most uniform, consistent thickness; fast for production; good for full-coverage pieces",
         ...
         "high"),
        ("application_effect", "method", "Brush application — variable thickness",
         ...
         "Glaze mixed to brushing consistency (specific gravity ~1.5-1.6, thicker than dipping), soft brush", "Variable thickness with visible brush marks; good for detail, accents, and layered effects",
         ...
         "high"),
        ("application_effect", "method", "Spray application — thinnest coat, requires PPE",
         ...
         "Spray gun or airbrush, glaze thinned to spraying consistency, PPE (respirator required)", "Thinnest, most even coat possible; excellent for gradients and color blending; good for large pieces",
         ...
         "high"),
        ("application_effect", "timing", "Drying between layers — prevent crawling",
         ...
         "Any layered glaze application", "Minimum 2-4 hours between layers; overnight drying is ideal; rushed application increases crawling risk significantly",
         ...
         "high"),
        ("application_effect", "measurement", "Specific gravity targets for glaze consistency",
         ...
         "Any glaze application", "Dipping: 1.4-1.5 specific gravity; Brushing: 1.5-1.6 specific gravity; Spraying: 1.3-1.4 specific gravity",
         ...
         "high"),
        ("application_effect", "preparation", "Glaze settling and remixing",
         ...
         "Any glaze that has been sitting unused", "Stir thoroughly before each use — heavier materials sink; a few minutes of stirring prevents uneven color and thickness",
         ...
         "high"),
        ("application_effect", "preparation", "Sieving glaze for smooth surfaces",
         ...
         "After mixing or remixing glaze, before application", "80 mesh sieve is standard for most glazes; 100 mesh or finer for smooth surfaces and spraying; removes lumps that cause pinholing and crawling",
         ...
         "high"),
        ("application_effect", "thickness_guideline", "Application thickness by glaze type",
         ...
         "Different glaze types", "Crystal glazes: thick application needed for crystal development; Shinos: thin application preferred (reduces crawling); Clear glazes: flexible, thin to medium; Textured/matte glazes: medium to thick; Glossy glazes: medium",
         ...
         "high"),
        ("application_effect", "technique", "Wax resist application for foot cleanup",
         ...
         "Apply to dry bisque foot before glazing", "Wax resists glaze on foot; makes cleanup easy; prevents pieces from sticking to kiln shelf",
         ...
         "high"),
        ("application_effect", "technique", "Layering wet glaze over dry layer — crawling risk",
         ...
         "Applying fresh wet glaze over a previously applied glaze layer that has dried fully", "High risk of crawling — the dry layer rehydrates unevenly, causing the glaze to pull away from the surface",
        ...
        # Troubleshooting (7)
        ("troubleshooting", "diagnosis", "Crawling diagnosis — bare patches in glaze",
         ...
         "high"),
        ("troubleshooting", "diagnosis", "Running diagnosis — glaze on kiln shelf",
         ...
         "Glaze flowed off piece, pooled at base, stuck to kiln shelf", "Check in order: (1) Was glaze applied too thick? Apply thinner next time. (2) Is the glaze over-fired? Check witness cones. (3) Does the glaze have a low melt point? May need kiln wash or catch plate. (4) Is the piece form too flat or horizontal? Glaze runs more on flat surfaces.",
         ...
         "high"),
        ("troubleshooting", "diagnosis", "Pinholing diagnosis — small pits in glaze surface",
         ...
         "Small pits or holes in fired glaze surface, rough to the touch", "Check in order: (1) Is bisque underfired? Fire bisque to cone 04 instead of cone 06 — this burns out more organic material. (2) Was firing too fast? Slow the rate, especially through 1000-1500°F. (3) Is glaze too thin? Apply slightly thicker. (4) Was there a hold at peak temperature? A 15-30 minute hold lets the glaze level and heal over. (5) Is the clay body high in organic material or grog?",
         ...
         "high"),
        ("troubleshooting", "diagnosis", "Crazing diagnosis — crackle pattern in glaze",
         ...
         "Fine crackle pattern in glaze, may appear immediately or develop over weeks/months", "Check in order: (1) Is the glaze COE (coefficient of thermal expansion) too high for the clay body? Switch to a better-matched glaze or adjust the recipe. (2) Did the kiln cool too fast? Slow cooling reduces crazing. (3) Is the piece thin-walled? Thin pieces are more susceptible. (4) Was the glaze applied too thick? Thinner application reduces stress.",
         ...
         "high"),
        ("troubleshooting", "diagnosis", "Shivering diagnosis — glaze popping off clay body",
         ...
         "Glaze flaking or popping off the clay body in chips, sharp edges", "Check in order: (1) Is the glaze COE too low? Increase sodium/potassium fluxes or reduce silica/alumina. (2) Is the glaze applied too thin? Thin application is more prone to shivering. (3) Is the clay body high in thermal expansion? Some bodies don't pair well with low-COE glazes. (4) Did the piece cool too fast? Rapid cooling can trigger shivering.",
         ...
         "high"),
        ("troubleshooting", "diagnosis", "Color off diagnosis — unexpected glaze color",
         ...
         "high"),
        ("troubleshooting", "diagnosis", "Dunting diagnosis — cracking during cooling",
        ...
        ("umf_interpretation", "ratio", "Silica:Alumina ratio predicts glaze surface",
         ...
         "UMF SiO2:Al2O3 ratio", "Below 3:1 = underfired or dry matt; 3:1 to 5:1 = matt; 5:1 to 10:1 = glossy; above 10:1 = runny",
         ...
         "high"),
        ("umf_interpretation", "ratio", "Flux:Alumina ratio predicts melt behavior",
         ...
         "UMF total fluxes (sum = 1.0) divided by Al2O3", "Below 2:1 = dry, underfired appearance; 2:1 to 3:1 = stable melt with good surface; above 3:1 = runny, potential defects",
         ...
         "high"),
        ("umf_interpretation", "flux_type", "Alkali vs alkaline earth flux ratio affects expansion and surface",
         ...
         "Ratio of alkali (KNaO) to alkaline earth fluxes in UMF", "High alkali (KNaO > 0.5): higher thermal expansion, more fluid, brighter colors, prone to crazing. Balanced (KNaO 0.2-0.4): most common for stable glazes. High alkaline earth (CaO+MgO > 0.7): lower expansion, stiffer melt, promotes matte surfaces",
         ...
         "high"),
        ("umf_interpretation", "stability", "UMF limit formulas for cone 10 glaze stability",
         ...
         "Cone 8-10 temperature range", "KNaO: 0.1-0.5; CaO: 0.2-0.8; MgO: 0.0-0.4; BaO: 0.0-0.3 (toxic); Al2O3: 0.2-0.5; SiO2: 2.5-5.0; B2O3: 0.0-0.3",
         ...
         "medium"),
        ("umf_interpretation", "diagnosis", "Reverse engineering glaze behavior from UMF",
         ...
         "medium"),
        ("umf_interpretation", "method", "UMF calculation method overview",
         ...
         "Any glaze recipe with known material analyses", "Step 1: Convert each material's weight percentage to oxide moles using molecular weights. Step 2: Sum all flux oxide moles (K2O, Na2O, CaO, MgO, ZnO, BaO, etc.). Step 3: Divide ALL oxide amounts by the total flux sum. Result: fluxes = 1.0, Al2O3 and SiO2 shown relative to flux.",
        ...
        ("material_substitution", "colorant", "Cobalt oxide vs carbonate substitution",
         ...
         "Recipe calls for cobalt oxide but only carbonate available, or vice versa", "Cobalt oxide → cobalt carbonate: multiply by 1.48 (oxide is 93% CoO, carbonate is 63% CoO). Cobalt carbonate → cobalt oxide: multiply by 0.68.",
         ...
         "high"),
        ("material_substitution", "colorant", "Copper oxide vs carbonate substitution",
         "Copper oxide (CuO) and copper carbonate (CuCO3) supply copper at different concentrations. Source: glaze-chemistry.md.",
         "Recipe calls for copper oxide but only carbonate available, or vice versa", "Copper oxide → copper carbonate: multiply by 1.55 (oxide is ~100% CuO, carbonate is ~64-65% CuO). Copper carbonate → copper oxide: multiply by 0.64.",
         ...
         "high"),
        ("material_substitution", "colorant", "Manganese dioxide vs carbonate substitution",
         ...
         "Recipe calls for manganese dioxide but only carbonate available, or vice versa", "Manganese dioxide → manganese carbonate: multiply by 1.62 (dioxide is ~100% effective MnO, carbonate is ~61-62% MnO). Manganese carbonate → manganese dioxide: multiply by 0.62.",
         ...
         "high"),
        ("material_substitution", "flux", "Whiting vs wollastonite substitution",
         ...
         "Recipe calls for whiting but only wollastonite available, or vice versa", "Whiting → wollastonite: multiply by 1.16 (whiting is 56% CaO, wollastonite is 48% CaO + 52% SiO2). Wollastonite → whiting: multiply by 0.86. IMPORTANT: when substituting wollastonite for whiting, reduce the silica in the recipe by approximately 6% to compensate for the silica wollastonite contributes.",
         ...
         "high"),
        ("material_substitution", "flux", "Barium vs strontium substitution",
         ...
         "Recipe calls for barium carbonate but want to avoid barium toxicity", "Barium carbonate → strontium carbonate: multiply by 0.75 (BaCO3 is 77.5% BaO, SrCO3 is 70% SrO). Strontium carbonate → barium carbonate: multiply by 1.33. Strontium is LESS toxic than barium.",
         ...
         "medium"),
        ("material_substitution", "feldspar", "Feldspar substitution principles",
         ...
         "high"),
        ("material_substitution", "complex", "Gerstley borate and Albany slip replacements",
         ...
         "Recipe calls for Gerstley Borate or Albany Slip", "Gerstley Borate replacements: Colemanite (calcium borate), boron frits (e.g., Ferro 3134), or calcined borax. Note: both Gerstley Borate and Colemanite are naturally variable materials. Albany Slip replacements: Alberta Slip, Seattle Slip, Sheffield Slip Clay Formula, or A.R.T. Albany Slip Synthetic. Matching depends on amount in recipe, temperature, and atmosphere.",
        ...
        ("clay_body_interaction", "iron", "Iron in clay body affects translucent glaze color",
         ...
         "Any translucent or semi-transparent glaze over an iron-bearing clay body", "Translucent glazes over iron-bearing clay will appear darker, warmer, or more amber than the same glaze over a white clay body. The effect is proportional to both the iron content in the clay and the translucency of the glaze.",
         ...
         "high"),
        ("clay_body_interaction", "iron", "Iron content in clay body darkens with firing temperature",
         ...
         "Iron-bearing clay body fired to higher temperatures", "As firing temperature increases from 600°C to 1025°C+, iron converts to hematite, darkening the clay body from light buff to reddish brown. This affects any glaze applied over the body, especially translucent glazes.",
         ...
         "high"),
        ("clay_body_interaction", "absorption", "Clay body porosity affects glaze application and fit",
         ...
         "high"),
        ("clay_body_interaction", "thermal_expansion", "Clay body thermal expansion must match glaze for fit",
         ...
         "Any glaze applied to a new or different clay body", "Each clay body has a specific coefficient of thermal expansion (CTE). A glaze that fits one body perfectly may craze on a body with lower expansion or shiver on a body with higher expansion. There is no universal 'food-safe' glaze — fit depends on the specific body-glaze combination.",
        ...
        ("surface_quality", "gloss_matt", "Converting matte to glossy by adjusting silica",
         ...
         "Matte glaze that you want to be glossier", "Add silica incrementally until the desired surface is achieved. Sue McLeod demonstrated that adding 40g silica per 100g of a matte glaze base (keeping all other materials constant) converted it from matte to glossy. The SiO2:Al2O3 ratio must move from the matte zone (3-5:1) toward the glossy zone (5-10:1).",
         ...
         "high"),
        ("surface_quality", "gloss_matt", "High alumina creates matte surfaces",
         ...
         "Glaze with Al2O3 above 0.4 in UMF (cone 8-10)", "High alumina content produces matte surfaces because alumina raises the melting point and stiffens the melt, preventing it from leveling into a smooth glossy surface. The surface appears soft-matt to dry-matt depending on the exact ratio and other oxides.",
         ...
         "high"),
        ("surface_quality", "opacity", "Opacity control: tin, zirconium, and titanium",
         ...
         "Need to create opaque or semi-opaque glaze", "Tin oxide (5-15%): produces the whitest, most opaque surface but is expensive. Zirconium silicate (Zircopax, 10-20%): effective opacifier, less expensive than tin, slightly blue-white tone. Titanium dioxide (5-10%): opacifies and promotes variegation/crystal formation, can produce blue-white or creamy tones depending on other oxides.",
         ...
         "high"),
        ("surface_quality", "flux_effect", "Calcium and magnesium promote different matte surfaces",
        ...
        ("kiln_troubleshooting", "diagnosis", "Firing too slow — causes and solutions",
         ...
         "Kiln takes longer than usual or cannot reach target cone", "Diagnosis in order of likelihood: (1) Aging elements — elements gradually increase resistance as they oxidize, producing less heat. Check with multimeter. (2) Low voltage — seasonal voltage drops (summer brownouts) or other appliances on the same circuit. Check voltage at the kiln during firing. (3) Overloading — too much mass or too many shelves slow the kiln. Space shelves to allow heat flow. (4) Lid not sealing — heat escapes through gaps.",
         ...
         "high"),
        ("kiln_troubleshooting", "diagnosis", "Firing too fast — causes and solutions",
         ...
         "Kiln reaches temperature before the programmed time", "Common causes: (1) New elements — fresh elements produce more heat than aged ones. (2) Higher voltage than expected — check voltage. (3) Light load — a kiln with few pieces heats faster. (4) Incorrect controller settings — verify the program matches your intended schedule.",
         ...
         "high"),
        ("kiln_troubleshooting", "diagnosis", "Temperature gradient in kiln — hot and cold spots",
         ...
         "high"),
        ("kiln_troubleshooting", "programming", "Kiln controller programming basics",
        ...
        ("glaze_mixing", "process", "Glaze mixing process from dry recipe",
         ...
         "high"),
        ("glaze_mixing", "water", "Water ratio and specific gravity for glaze consistency",
         ...
         "high"),
        ("glaze_mixing", "testing", "Test tile methodology for glaze evaluation",
         ...
         "high"),
        ("glaze_mixing", "storage", "Glaze storage and shelf life",
        ...
        ("ergonomics", "workstation", "Pottery studio workstation height requirements",
         ...
         "high"),
        ("ergonomics", "handling", "Material handling safety in pottery studio",
         ...
         "high"),
        ("ergonomics", "repetitive_motion", "Repetitive motion prevention for potters",
    ...
    try:
        ...
        if external_rules:
            ...
            filtered_hardcoded = [r for r in chemistry_rules if (r[0], r[1], r[2]) not in external_keys]
            chemistry_rules = external_tuples + filtered_hardcoded
            logger.info(f"Loaded {len(external_tuples)} chemistry rules from ceramics-foundation, {len(filtered_hardcoded)} from defaults")
    except ImportError:
        logger.debug("ceramics-foundation data_loader not available, using hardcoded chemistry rules")

    for rule in chemistry_rules:
        cursor.execute('''INSERT INTO chemistry_rules
...
def compute_compatibility(glaze_a_name: str, glaze_b_name: str, db_path: str = None) -> dict:
...
def compute_compatibility_from_properties(glaze_a: dict, glaze_b: dict, name_a: str, name_b: str) -> dict:
    ...
    if is_shino_a != is_shino_b:
        score -= 15
        factors.append("Shino over non-Shino: moderate crawling risk (thin application recommended)")
    ...
    if food_safe_a and food_safe_b:
        if food_safe_a.lower() in ('yes', 'true') and food_safe_b.lower() in ('yes', 'true'):
